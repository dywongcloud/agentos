...
mod source_build {
    ...
    enum TargetEnv {
        ...
        Musl,
        Sgx,
        /// Apple's iOS/tvOS/watchOS *Simulator* targets (e.g.
        /// `aarch64-apple-ios-sim`, `x86_64-apple-ios-sim`) report
        /// `CARGO_CFG_TARGET_ENV=sim` -- distinct from a bare device target
        /// (`aarch64-apple-ios`, `CARGO_CFG_TARGET_ENV=""`) only in that it
        /// links against the Simulator runtime instead of the device one;
        /// nothing in this build script's C-toolchain logic below
        /// distinguishes them (the one downstream match on `TargetEnv` only
        /// ever special-cases `Gnu` on Windows), so this variant exists
        /// purely to stop the match below from panicking on a real,
        /// increasingly common Rust target triple.
        Sim,
    }
