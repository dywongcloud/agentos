                ...
                loop {
                    ...
                    match source.pop_frame() {
                        Ok(Some(frame)) => {
                            ...
                            if actual < expected {
                                // Clamped, mirroring the decode-side FramePacer's
                                // `pts_delta.min(frame_time * 2)` bound: an
                                // anomalous PTS jump (e.g. a stale boot-era frame
                                // anchoring base_pts while fresh frames carry
                                // time-since-boot timestamps -- live-witnessed as
                                // an hours-long sleep here, i.e. a permanent
                                // black screen for every subscriber) must never
                                // schedule a frame further than a beat into the
                                // future. 100ms = 3 frames at 30fps: enough to
                                // pace any real cadence, never enough to stall.
                                const MAX_PACING_SLEEP: Duration = Duration::from_millis(100);
                                thread::sleep((expected - actual).min(MAX_PACING_SLEEP));
                            }
