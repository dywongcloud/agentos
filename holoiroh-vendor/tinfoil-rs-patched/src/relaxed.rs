...
impl<'a> RelaxedChat<'a> {
    ...
    pub async fn create_stream(&self, body: impl Into<Value>) -> Result<RelaxedStream> {
        let mut body = body.into();
...
pub type RelaxedStream = std::pin::Pin<Box<dyn Stream<Item = Result<RelaxedStreamChunk>> + Send>>;
...
mod tests {
    ...
    fn http_error_classifies_status_codes() {
        ...
        let rate_limit = http_error(reqwest::StatusCode::TOO_MANY_REQUESTS, b"rate-limited");
