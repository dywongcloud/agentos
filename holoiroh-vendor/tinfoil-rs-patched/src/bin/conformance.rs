    ...
    while i < args.len() {
        match args[i].as_str() {
            ...
            "--certificate-identity" => {
                ...
                cert_identity = Some(
                    args.get(i)
                        .ok_or("--certificate-identity requires a value")?
                        .clone(),
                );
            }
            ...
            "--trusted-root" => {
                ...
                trusted_root = Some(
                    args.get(i)
                        .ok_or("--trusted-root requires a value")?
                        .clone(),
                );
            }
...
fn verify_message_signature(
    ...
    let msg_sig = bundle
        ...
        .ok_or("Missing messageSignature")?;
    let sig_b64 = msg_sig
        .get("signature")
        .and_then(|s| s.as_str())
    ...
    match key_info.curve_oid.as_deref() {
        Some(OID_P384) => {
            let verifying_key = p384::ecdsa::VerifyingKey::from_sec1_bytes(&key_info.raw_bytes)
...
fn verify_rekor_entry(
    ...
    let timestamp_for_key = if entry_version == "0.0.1" {
        entry.get("integratedTime").and_then(|t| {
            t.as_str()
                .and_then(|s| s.parse::<u64>().ok())
                .or(t.as_u64())
        })
...
fn extract_rekor_cert_der(body_json: &serde_json::Value) -> Result<Vec<u8>, String> {
    let kind = body_json.get("kind").and_then(|k| k.as_str()).unwrap_or("");
...
fn extract_rekor_signature(body_json: &serde_json::Value) -> Result<Vec<u8>, String> {
    ...
    let sig_b64 = match kind {
        ...
        "intoto" => {
            ...
            return decode_b64(&inner_str).map_err(|e| format!("Decode intoto sig inner: {e}"));
        }
        "hashedrekord" => {
            let inner = hashedrekord_inner_spec(spec);
            inner
...
fn verify_key_binding(key_path: &str, body: &[u8]) -> Result<(), String> {
    ...
    let inner = match kind {
        "hashedrekord" => hashedrekord_inner_spec(body_json.get("spec").ok_or("Missing spec")?),
...
fn verify_scts(cert_der: &[u8], trusted_root_json: &str) -> Result<(), String> {
    let cert = Certificate::from_der(cert_der).map_err(|e| format!("Parse cert for SCT: {e}"))?;
    ...
    for ca in &fulcio_cas {
        ...
        let verified = match sig_algo_oid.as_str() {
            ...
            "1.2.840.10045.4.3.3" => match curve_oid.as_deref() {
                ...
                Some("1.3.132.0.34") => verify_ecdsa_p384(&tbs_bytes, sig_bytes, issuer_pubkey),
                _ => continue,
            },
