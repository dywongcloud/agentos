...
pub async fn verify_repo(repo: &str) -> Result<SigstoreResult> {
    ...
    // 1. Fetch one release identity and derive its digest.
    let release_tag = github::fetch_latest_tag(repo).await?;
    let release_digest = github::fetch_digest(repo, &release_tag).await?;
    ...
    verify_certificate_identity(&cert_info, repo, &release_tag)?;
...
fn verify_certificate_identity(
    ...
    if !re.is_match(&cert_info.subject_workflow) {
        return Err(Error::SigstoreVerification(format!(
            "Certificate workflow does not match expected release. Expected: {}@refs/tags/{}, Got: {}",
            expected_repo, expected_tag, cert_info.subject_workflow
        )));
    }
...
fn extract_measurement_from_bundle(
