...
/// bug). Not automatable-by-touch: `@GestureState` can only be mutated by a
/// real `UIGestureRecognizer` in flight, but the actual rendering/diffing
/// cost a pinch or pan gesture triggers is identical to varying the plain
/// `@State` `zoomScale`/`panOffset` at the same frequency (both feed the same
/// `liveScale`/`liveOffset` computation `videoOverlay` renders from) -- so a
/// synthetic high-frequency state-update loop exercises the SAME code path a
/// human's fingers would, without requiring device touch-automation tooling
/// this session doesn't have. `CADisplayLink` measures the real per-frame
/// cost this drives, on real hardware.
///
