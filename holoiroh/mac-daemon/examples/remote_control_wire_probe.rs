...
fn main() {
    ...
    let take = ClientMessage::RemoteControl {
        event: RemoteControlEvent::TakeControl,
    };
    let j = rt(&take);
    println!("take  -> {j}");
    assert!(
        j.contains("\"type\":\"remote_control\""),
        "wrong type tag: {j}"
    );
    assert!(
        j.contains("\"action\":\"take_control\""),
        "wrong action tag: {j}"
    );

    let mv = ClientMessage::RemoteControl {
        event: RemoteControlEvent::Move { x: 0.5, y: 0.25 },
    };
    let j = rt(&mv);
    println!("move  -> {j}");
    assert!(
        j.contains("\"action\":\"move\"") && j.contains("\"x\":0.5"),
        "move shape: {j}"
    );
    ...
    println!("click -> {j}");
    assert!(
        j.contains("\"button\":\"left\"") && j.contains("\"count\":2"),
        "click shape: {j}"
    );

    let btn = ClientMessage::RemoteControl {
        event: RemoteControlEvent::Button {
            x: 0.3,
            y: 0.4,
    ...
    let text = ClientMessage::RemoteControl {
        event: RemoteControlEvent::Text {
            text: "hi team".into(),
        },
    };
    let j = rt(&text);
    assert!(
        j.contains("\"action\":\"text\"") && j.contains("\"text\":\"hi team\""),
        "text shape: {j}"
    );

    let key = ClientMessage::RemoteControl {
        event: RemoteControlEvent::Key {
            key: "return".into(),
            down: true,
        },
    };
    ...
    let release = ClientMessage::RemoteControl {
        event: RemoteControlEvent::ReleaseControl,
    };
    let j = rt(&release);
    assert!(
        j.contains("\"action\":\"release_control\""),
        "release shape: {j}"
    );
    ...
    println!(
        "remote_control_wire_probe: OK -- every remote-control action round-trips as the client expects."
    );
}
