...
fn main() {
    ...
    let running = ServerMessage::TaskActive {
        paused: false,
        queued: 0,
    };
    ...
    assert!(
        json.contains("\"type\":\"task_active\""),
        "wrong type tag: {json}"
    );
    assert!(
        json.contains("\"paused\":false"),
        "missing paused field: {json}"
    );
    assert!(
        json.contains("\"queued\":0"),
        "missing queued field: {json}"
    );
    ...
    let paused = ServerMessage::TaskActive {
        paused: true,
        queued: 2,
    };
    ...
    println!(
        "task_active_wire_probe: OK -- task_active serializes/deserializes as the client expects."
    );
}
