...
fn main() {
    ...
    assert!(
        br.x > tl.x && br.y > tl.y,
        "display must have positive extent"
    );
    assert!(
    ...
    assert!(
        (over.x - br.x).abs() < 0.01 && (over.y - tl.y).abs() < 0.01,
        "coords must clamp to 0..1"
    );
    ...
    println!(
        "after move(0.5,0.5): cursor=({:.0},{:.0}) expected~({:.0},{:.0})",
        cur.x, cur.y, c.x, c.y
    );
    ...
    assert!(
        dx < 3.0 && dy < 3.0,
        "cursor did not land at the mapped center (dx={dx}, dy={dy})"
    );
    ...
    println!(
        "remote_input_probe: OK -- injection moved the real cursor to the mapped display point."
    );
}
