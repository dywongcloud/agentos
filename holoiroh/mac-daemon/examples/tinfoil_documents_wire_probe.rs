...
use holoiroh_daemon::tinfoil_documents::{
    convert_documents, ConvertMode, DocumentInput, MAX_FILES_PER_REQUEST, MAX_FILE_BYTES,
};
...
async fn main() {
    ...
    assert!(rj.contains("\"type\":\"process_document\""), "wrong type tag: {rj}");
    ...
    assert!(oj.contains("\"type\":\"document_processed\""), "wrong type tag: {oj}");
    ...
    assert!(fj.contains("\"type\":\"document_process_failed\""), "wrong type tag: {fj}");
    ...
    let err = convert_documents("fake-key", &empty_files, ConvertMode::Text)
        .await
    ...
    let too_many: Vec<DocumentInput> = (0..MAX_FILES_PER_REQUEST + 1)
        ...
        .collect();
    let err = convert_documents("fake-key", &too_many, ConvertMode::Text)
        .await
    ...
    let err = convert_documents("fake-key", &empty_file, ConvertMode::Text)
        .await
        .expect_err("a 0-byte file must be rejected, not sent");
    ...
    let err = convert_documents("fake-key", &oversized, ConvertMode::Text)
        .await
