...
fn main() {
    ...
    assert_eq!(events.len(), 2, "escape down+up must record exactly 2 events, got {events:?}");
    assert_eq!(events[0], (53, true, 0), "escape keycode must be 53 (kVK_Escape), flags 0: {events:?}");
    assert_eq!(events[1], (53, false, 0), "escape up must match: {events:?}");
    ...
    let events = remote_input::take_applied_keys();
    assert_eq!(events.len(), 4, "cmd+c+c+cmd must record exactly 4 events, got {events:?}");
    ...
    // carries none.
    assert_eq!(events[0], (55, true, MASK_COMMAND), "cmd-down must report itself as active: {events:?}");
    assert_eq!(events[1].0, 8, "'c' must resolve to keycode 8 (kVK_ANSI_C): {events:?}");
    assert_eq!(events[1].1, true);
    assert_eq!(events[1].2 & MASK_COMMAND, MASK_COMMAND, "'c' pressed while cmd held must carry MaskCommand: {events:?}");
    assert_eq!(events[2], (8, false, MASK_COMMAND), "'c' release must still carry the Command flag: {events:?}");
    ...
    let events = remote_input::take_applied_keys();
    let v_event = events.iter().find(|e| e.0 == 9).expect("'v' event must be recorded");
    assert_eq!(v_event.2 & MASK_COMMAND, 0, "release_all must have cleared cmd -- 'v' must NOT carry MaskCommand: {events:?}");
    println!("release_all: OK -- abandoned modifier state is cleared, does not leak into the next key");
    ...
    for alias in ["opt", "option", "alt"] {
        ...
        let events = remote_input::take_applied_keys();
        assert_eq!(events, vec![(58, true, MASK_ALTERNATE)], "'{alias}' must resolve to the Option keycode (58): {events:?}");
