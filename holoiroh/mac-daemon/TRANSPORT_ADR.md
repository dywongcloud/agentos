...
- **Scope:** the media (video) plane only. The control plane
  (prompts/status/`TaskEnvelope`) is a separate iroh ALPN. It is out of scope
...
client. It asks which candidate to commit to:
...
2. **Fallback — native WebRTC:** DTLS-SRTP media with offer/answer exchange
   over an authenticated iroh signaling stream. This fallback applies only if
   the primary path misses the PRD §12.1 latency targets (Remote View active
   in <2s median).
...
> "H.264-over-iroh" as OQ-5 specifies? If yes, a separate, custom-built QUIC
> video stream over iroh (or the WebRTC fallback) becomes unnecessary.
...
custom QUIC video stream over iroh is needed. WebRTC is not needed for the
primary path either.** The transport is therefore effectively **already chosen
and wired** on the Mac side. The only remaining media-plane work is the iOS
**subscribe** side, which is a separate row (the iroh-FFI row,
`holoiroh-ios-iroh-ffi-integration`).

This investigation produced one concrete code change (see "Encoder correction"
below): the daemon now selects the **hardware VideoToolbox** H.264 encoder
instead of the software openh264 encoder that `main.rs` previously hardcoded.

## Evidence

All paths below are in the vendored `iroh-live` checkout pinned by
`mac-daemon/Cargo.toml` (rev `5f95758fcd1450e443a9134c9d9342bcc3957b85`). This
ADR reads them directly from
...
connections**."* Its ALPN is `moq-lite-04` (`iroh-moq/src/lib.rs:35`). iroh-moq
establishes a session via `endpoint.connect(addr, ALPN)` →
...
streams on the iroh endpoint. A hand-rolled, custom QUIC video framing over
iroh would re-implement what MoQ already gives us, only worse and unmaintained.
...
codec identity is H.264 either way. The iOS decode path
(`AVSampleBufferDisplayLayer`, `ios/.../Video/VideoRenderView.swift`) is
therefore unaffected by the choice.

`VideoCodec::best_available()` prefers hardware over software. It returns
...
`default-features = false`). The `videotoolbox` feature is therefore on.
...
`VideoCodec::VtbH264` exists. `moq_media::publish::add()` maps it to
`codec::VtbEncoder` (`moq-media/src/publish.rs:998-999`).

### 4. The daemon already publishes this transport (Mac side is wired)

`main.rs` performs four steps to publish this transport:

1. Brings up `Live::from_env().await?.spawn()`.
2. Mounts MoQ and the control ALPN on one shared `Router`
   (`live.register_protocols(router_builder)`).
3. Attaches the ScreenCaptureKit source to a `LocalBroadcast`.
4. Calls `live.publish(BROADCAST_NAME, &broadcast)` — which is
   `moq.publish(name, broadcast.producer())` (`iroh-live/src/live.rs:204-206`).
   This call announces the broadcast over MoQ to every connected or future peer.

The subscriber side of the same library (`Live::subscribe` →
`Subscription::media_with_decoders`, `iroh-live/src/live.rs:229-279`,
`iroh-live/src/subscription.rs:62-81`) is the symmetric decode API the iOS
client will call through the FFI bridge.

## Why not the alternatives

**Custom, low-level QUIC video stream over iroh — rejected.** OQ-5 floats this
only "if the primary path misses the PRD's specific latency targets." There is
no evidence it does. MoQ-over-iroh is *already* per-rendition independent QUIC
streams (Evidence §1), i.e. the exact primitive a custom stream would provide.
Building bespoke framing, congestion, and keyframe-request logic on raw
`iroh::Connection` streams would duplicate `iroh-moq` and `moq-media` as an
unmaintained fork. Those crates already provide encode, catalog, rendition
selection, and `NetworkSignals`-driven adaptive bitrate (`subscription.rs:54-56`).
This approach becomes justified only if evidence traces a *measured* latency
miss specifically to MoQ overhead. That evidence requires the live daemon to
run past the macOS TCC gate (blocked; see below). Deferring it is correct: it
is a contingency, not the plan.
...
**Finding:** `main.rs` hardcoded `VideoCodec::H264` — the **software
openh264** encoder (`moq-media/src/publish.rs:995`:
`VideoCodec::H264 => codec::H264Encoder`) — despite VideoToolbox being compiled
in (Evidence §3). So the daemon did *not* actually use VideoToolbox, contrary
to the "H.264/VideoToolbox on macOS" phrasing in the README and OQ-5's
"VideoToolbox-encoded frames" intent. Software H.264 at 720p means higher CPU
and higher encode latency — directly counter to the PRD §12.1 <2s / low-latency
goal.

**Fix:** `main.rs` now selects the encoder via
`VideoCodec::best_available().unwrap_or(VideoCodec::H264)`. This function
returns `VtbH264` (hardware VideoToolbox) on this macOS build. It falls back to
software openh264 only if no hardware encoder is available. This matches
`iroh-live`'s own reference CLI. That CLI defaults the codec via
`VideoCodec::parse_or_best(None)` → `best_available()`
(`iroh-live-cli/src/args.rs:129-130`, `rusty-codecs/src/codec.rs:253-260`). The
wire codec stays H.264/AVC on either branch. The iOS decoder is therefore
unchanged. The fallback causes only a graceful CPU-cost degradation, never a
format break.

**Witness:** `cargo build -p holoiroh-daemon` is clean after the change
(`Finished dev [unoptimized + debuginfo] in 5.18s`). The feature-graph proof in
Evidence §3 shows that `best_available()` deterministically resolves to
`VtbH264` in this build. A *runtime* confirmation of the emitted encoder id
(`h264-vtb`) additionally requires the daemon to run past the macOS Screen
Recording/Accessibility TCC preflight. This preflight is the standing external
blocker tracked by `holoiroh-user-action-grant-tcc-and-run-daemon` and the
`holoiroh-e2e-witness-blocked-by-tcc-permissions` mutable. Apple provides no CLI
to grant TCC access. Only interactive System Settings can grant this access.

## Consequences

- The Mac-side media transport for Remote View is **chosen and wired**:
...
- The one media-plane piece still genuinely unbuilt is the **iOS subscribe**
  ...
  Swift through the Rust FFI bridge (`ios-bridge/`). The tracking item for this
  work is `holoiroh-ios-iroh-ffi-integration`. The renderer that consumes those
  decoded frames already exists (`ios/.../Video/VideoRenderView.swift`).
