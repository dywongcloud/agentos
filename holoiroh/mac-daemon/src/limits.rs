...
//! re-hardcode the numbers. This practice keeps the PRD table and the code
//! from drifting silently out of sync.
//!
...
//! existing `control_channel.rs`/`holo_bridge/control.rs` code predates PRD
//! 10.4. Some of the limits below need wire-protocol or session-state-machine
//! surface this codebase does not have today. See each constant's own doc
//! comment for the specific gap. See `holoiroh/README.md`'s "Session & rate
//! limits (PRD 10.4)" section for the consolidated table.
//!
//! Concretely, as of this module's introduction:
//!
...
//!   That same struct's pre-existing `busy`/`queue` mechanism already
//!   enforces [`MAX_ACTIVE_TASKS_PER_MAC`] (see that constant's doc).
//! - **Scaffolded:** the constant and helper type exist and are real,
//!   independently exercised via `cargo run --example`, but not wired into
//!   a live call site. Wiring them in needs machinery this codebase doesn't
//!   have yet: a task/session state machine, a wire-schema timestamp field,
//!   or a manual-input channel. This applies to [`SESSION_LIFETIME_MAX_SECS`] /
...
///
/// A task request older than this should be rejected, not acted on. A
/// stale request might finally reach the Mac after a long network stall
/// or reconnect. That stale request should not execute silently.
///
/// **Not wired to a real enforcement point.** `crate::control_channel`'s
/// wire schema (`ClientMessage::Prompt`/`VoiceTranscript`, see
/// `holoiroh/PROTOCOL.md`) carries no timestamp field today. There is no
/// `sent_at` field to compute age against.
///
/// Real enforcement requires a wire-schema change: adding a
/// `sent_at`/`expires_at` field to `ClientMessage`. That change is out of
/// this module's scope. See the `holoiroh-task-envelope-protocol` PRD row
/// for the tracked fix. That PRD row already specifies an `expires_at`
/// field on the richer task-envelope schema PRD 7.1 defines.
#[allow(dead_code)]
...
/// `examples/limits_probe.rs`). Not yet called from a live session-tracking
/// call site. This codebase has no persistent "session" object spanning
/// multiple control-channel connections or tasks yet.
/// [`crate::control_channel::ControlChannel::accept`] handles each
/// accepted `iroh` connection in `control_channel.rs` inline, with no
/// session struct threaded through it.
#[allow(dead_code)]
pub const SESSION_LIFETIME_MAX_SECS: u64 = 10 * 60;

/// Tracks the age of one active session against [`SESSION_LIFETIME_MAX_SECS`].
///
/// Real, independently-usable timer type, not a stub. Construct one with
/// [`SessionTimer::start`]. Call [`SessionTimer::is_expired`] or
...
impl SessionTimer {
    ...
    /// Starts a new session timer with an explicit max lifetime. This
    /// mainly helps callers, including `examples/limits_probe.rs`, that
    /// need a short duration to observe real expiry without waiting 10
    /// minutes.
    ...
    /// True once this session's age reaches at least `max` (default: PRD
    /// 10.4's 10 minutes).
...
///
/// Scaffolded via [`ApprovalToken`], independently exercised. Not yet wired
/// into a live call site. This codebase has no approval-gating flow yet.
/// See the `holoiroh-sensitive-app-approval-gating` and `holoiroh-p014-*`
/// PRD rows for where a real approval flow would issue and consume these
/// tokens.
#[allow(dead_code)]
pub const APPROVAL_TOKEN_TTL_SECS: u64 = 60;

/// A single-use, single-task-scoped approval credential per PRD 10.4. It is
/// valid for exactly one task and one action. It expires after
/// [`APPROVAL_TOKEN_TTL_SECS`] (60s) if unused.
///
/// Real, independently-usable type. [`ApprovalToken::issue`] mints one
/// token scoped to a `task_id`. [`ApprovalToken::consume`] enforces both
/// the TTL and the single-use constraint atomically. A token can be
/// consumed at most once, and only before it expires.
#[allow(dead_code)]
#[derive(Debug)]
...
pub enum ApprovalTokenError {
    /// The token's TTL ([`APPROVAL_TOKEN_TTL_SECS`] by default) elapsed.
    Expired,
    /// An earlier action already consumed the token. One approval token
    /// authorizes exactly one action, per PRD 10.4.
    AlreadyConsumed,
    /// The token was issued for a different `task_id` than the `task_id`
    /// in this `consume` call. One approval token authorizes exactly one
    /// task.
    WrongTask,
}
...
impl ApprovalToken {
    ...
    ///
    /// Uses `compare_exchange` on the `consumed` flag. Two concurrent
    /// `consume` calls racing on the same token can never both succeed.
    /// This is exactly the "one task + one action" guarantee PRD 10.4
    /// requires.
    ...
    /// True if [`consume`](Self::consume) could still succeed on this
    /// token: it is unexpired and not yet consumed. This is a non-mutating
    /// check. It helps a UI-side "is this still valid" display without
    /// spending the token.
...
/// flag and `queue`. A second `Prompt` or `VoiceTranscript` arriving while
/// one is in flight is queued, never run concurrently -- see that
/// struct's own doc comment, which now cites this constant explicitly.
///
/// This constant exists so that mechanism is traceable back to the PRD
/// requirement it satisfies. It is not left as an implicit "one at a
/// time" design decision with no named limit backing it.
...
/// `HoloBridge::start` takes a single `events_tx` at construction time.
/// Because of this, only one connection's events are ever delivered
/// correctly. That is a side effect of event routing, not an
...
/// logic independently. `ControlChannel` has no shared "is a controller
/// already active" guard anywhere.
///
/// A second simultaneous connection from an already-allowlisted device is
/// **not rejected**. It follows the same path as the first connection:
/// - It passes
///   [`crate::control_channel::ControlChannel::authenticate`]'s fast path
///   (allowlisted device, no PIN needed), exactly like the first
///   connection.
/// - It gets its own greeting.
/// - It calls `self.bridge.replace_event_sink(events_tx.clone())` on
///   every message it sends.
///
/// That call silently steals the shared
/// [`crate::holo_bridge::HoloBridge`]'s single event sink out from under
/// whichever connection had it before. `HoloControlBridge::replace_event_sink`'s
/// own doc already describes this exact reconnect-redirect mechanism. It
/// was designed for "old connection dropped, new one takes over," not for
/// "two connections both alive, second one silently wins."
///
/// So today: two controllers CAN coexist. The daemon does not reject the
/// second one. Only the most recent sender's connection receives
/// `ControlEvent`s. The older connection is not torn down; it is just
/// silently starved of replies.
///
/// This is a real gap, not a full implementation of PRD 10.4's
/// max-1-controller requirement. See [`MAX_ACTIVE_CONTROLLERS_PER_MAC`]'s
/// value and this doc as the honest record of that gap.
///
/// A real fix is tracked as a follow-up, not done here, because it
/// changes accept-time rejection behavior for an already-allowlisted
/// device and needs product sign-off on the resulting UX (for example,
/// should the *new* or the *old* connection win). That fix would:
/// - Add an `Arc<Mutex<Option<String>>>` "current controller" guard to
///   `ControlChannel`, checked and set at the top of `accept` after
///   `authenticate` succeeds.
/// - Reject a second device with a `ServerMessage::error`, before
///   `accept_bi`'s stream is used for anything else.
#[allow(dead_code)]
...
/// is actually clamped, not passed through.
///
/// Not yet called from `holo_bridge::control`, because that module has no
/// per-task deadline concept today. `HoloControlBridge::run_prompt` runs
/// `send_and_stream` to completion with no timeout at all.
///
/// Wiring a real deadline there is tracked as a follow-up. That would
/// need `tokio::time::timeout` wrapping the `send_and_stream` call. Doing
/// so changes error-handling and cancellation behavior on timeout, and
/// deserves its own careful pass rather than a drive-by wrap here.
#[allow(dead_code)]
...
/// counter is constructed per task turn. It increments on every progress
/// event forwarded from `holo serve`. The turn is aborted with a
/// [`crate::holo_bridge::control::ControlEvent::Error`] if the cap would
/// be exceeded.
///
/// The value is 500, raised from the original 100. An A2A `Working`
/// update is NOT one agent action: `holo serve` streams several status
/// updates per real step (observation, thought, tool call). A live
/// multi-app task on the tinfoil fallback (kimi-k2-6) was witnessed
/// emitting 100 updates within 3.5 minutes of legitimate, non-looping
/// work. That volume latched the old cap and, with the old
/// suppress-everything latch, swallowed the turn's real final answer. 500
/// still bounds a genuinely runaway agent, while giving real tasks the
/// headroom their event volume needs.
pub const AGENT_ACTION_CAP_DEFAULT: u32 = 500;

/// Counts agent actions within one task. Refuses further actions once
/// [`AGENT_ACTION_CAP_DEFAULT`] (or an explicit override) is reached.
///
/// Uses an [`AtomicU32`] with `fetch_update`. This means concurrent
/// callers can never both observe "under the cap" and both succeed past
/// it. `HoloControlBridge`'s own `busy`/`queue` locking already uses this
/// same race-safety discipline elsewhere in this crate.
#[derive(Debug)]
...
impl ActionCounter {
    ...
    /// A counter capped at an explicit `cap`, for a task that negotiated a
    /// different limit than the default. PRD 10.4 names 100 as the
    /// *default*, not a hardcoded universal ceiling.
    ...
    /// exceed it. The count is **not** incremented on a refused call. A
    /// caller that keeps calling `try_record` after a refusal gets the
...
/// `VoiceTranscript`, `Stop`, and `Pin`. All four are agent-directed or
/// pairing messages. There is no `manual_input` message type or stream at
/// all.
///
/// PRD 7.1 describes a richer 6-stream protocol, which includes a
/// dedicated `manual_input` stream for post-Take-Control keyboard and
/// mouse events. That protocol is tracked separately under the
/// `holoiroh-task-envelope-protocol` PRD row, and supersedes today's
/// `PROTOCOL.md`.
///
/// Real rate-limiting needs that channel to exist first. This constant is
/// recorded now, so the eventual implementation has an authoritative
/// value to import rather than re-deriving it from the PRD text again.
#[allow(dead_code)]
