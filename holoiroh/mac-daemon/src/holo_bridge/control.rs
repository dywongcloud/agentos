...
mod tests {
    ...
    /// Builds a `HoloControlBridge` pointed at a base URL with no listener (a reserved
    /// TEST-NET-1 address per RFC 5737, guaranteed not to route) so `send_and_stream` fails
    /// fast and deterministically with a connection error rather than actually reaching a
    /// `holo serve` instance -- exactly what these tests need to observe queue/busy state
    /// transitions without a live backend.
    fn unreachable_bridge() -> (HoloControlBridge, mpsc::UnboundedReceiver<ControlEvent>) {
        let (tx, rx) = mpsc::unbounded_channel();
        let client = A2aClient::new("http://192.0.2.1:1".to_string(), "test-token".to_string());
        (HoloControlBridge::new(client, "holo", tx), rx)
    }

    #[tokio::test]
    async fn second_prompt_while_first_in_flight_is_queued_not_run_immediately() {
        let (bridge, mut rx) = unreachable_bridge();

        // Fire both prompts concurrently, exactly the race the task describes: a second
        // `Prompt` arriving while the first is still in flight (still awaiting its failed
        // connection attempt against the unreachable client).
        tokio::join!(
            ...
            async {
                // Give the first call a moment to acquire `busy` before the second is sent,
                // so this test deterministically exercises the "queued" branch rather than
                // racing which one wins the lock first.
        ...
        assert!(
            queued.is_some(),
            "expected a Queued event for the second (racing) prompt, got: {events:?}"
        );
        if let Some(ControlEvent::Queued { ahead, .. }) = queued {
            assert_eq!(*ahead, 0, "second prompt was the only one queued behind the first");
        }

        // Both request_ids must eventually reach a terminal Error (the unreachable client
        // fails every real send) -- neither is silently dropped, and the queued one really
        // did get its own turn once the first turn's `run_prompt` returned (see
        // `handle_prompt`'s `self.drain_queue().await` call after `run_prompt`).
        let terminal_ids: Vec<&str> = events
            ...
            .collect();
        assert!(terminal_ids.contains(&"first"), "events: {events:?}");
        assert!(terminal_ids.contains(&"second"), "events: {events:?}");

        // Never two Prompt turns in flight at once against the same client: the second
        // prompt's Ack must not appear before the first prompt's terminal Error, i.e. it was
        // truly queued and sequenced, not raced.
        ...
        let second_run_started_pos = events
            .iter()
        ...
        assert!(
            second_run_started_pos > first_error_pos,
            "queued prompt's terminal event must come after the in-flight prompt's terminal event: {events:?}"
        );
    }

    #[tokio::test]
    async fn stop_drains_queued_prompts_with_a_terminal_done_event_each() {
        let (bridge, mut rx) = unreachable_bridge();
        ...
        tokio::join!(
            ...
            async {
                tokio::time::sleep(Duration::from_millis(20)).await;
                // Two more prompts arrive while "running" is in flight -- both should queue.
                bridge
                    .handle(ControlMessage::Prompt {
            ...
            async {
                // Stop arrives after both are queued but (deterministically, given the
                // unreachable client's connect timeout is much longer than a few ms) before
                // "running" has reached its own terminal state.
        ...
        for id in ["queued-1", "queued-2"] {
            let done = events.iter().find(|e| {
                matches!(e, ControlEvent::Done { request_id, status: DoneStatus::Canceled, .. } if request_id == id)
            });
            assert!(
                done.is_some(),
                "expected queued prompt {id} to get a terminal Done{{Canceled}} event from Stop, got: {events:?}"
            );
        }

        // The queue itself must actually be empty afterwards -- Stop's drain must not leave
        // either prompt to later be dispatched to `holo serve` behind the user's back.
        assert!(
            bridge.queue.lock().expect("queue lock poisoned").is_empty(),
            "queue must be empty after Stop drains it"
        );
    }

    #[tokio::test]
    async fn queued_prompt_reports_correct_ahead_count_for_multiple_queued() {
        let (bridge, mut rx) = unreachable_bridge();
    ...
    #[test]
    fn control_event_queued_serializes_to_status_shaped_wire_message_via_control_channel() {
        // This module intentionally doesn't depend on `crate::control_channel` (see module
        // doc: transport-agnostic), so the actual wire-shape assertion for
        // `{"type":"status","text":"queued, N ahead"}` lives in
        // `control_channel::tests::control_event_queued_maps_to_status_with_ahead_count`.
        // This test only pins the internal `ControlEvent::Queued` shape itself so a future
        // field rename here is caught at this layer too.
        let event = ControlEvent::Queued {
            request_id: "r1".into(),
            ahead: 3,
        };
        match event {
            ControlEvent::Queued { request_id, ahead } => {
