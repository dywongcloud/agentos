...
pub enum ControlEvent {
    ...
    /// A daemon-wide status update not scoped to any particular `request_id` -- e.g. `holo
    /// serve` crashed and was restarted by the health-check supervisor (see
    /// `crate::holo_bridge::health`). Flows through the same `emit`/`events_tx` path as every
    /// other `ControlEvent`, so it reaches the currently-connected peer via
    /// `crate::control_channel::ControlChannel` exactly like an A2A-derived event does.
...
pub struct HoloControlBridge {
    /// Wrapped in the same `std::sync::RwLock` pattern as `events_tx` below (see that field's
    /// doc for why `std`, not `tokio`) so [`HoloBridge::restart_process`](crate::holo_bridge::HoloBridge::restart_process)
    /// can swap in a freshly-constructed client after a `holo serve` crash-restart -- the old
    /// client's bearer token stops being valid the moment the old process exits (`process`
    /// module always generates a fresh token per spawn). `A2aClient` is cheap to `.clone()`
    /// out of the lock for use (see its own doc), so call sites never hold this lock across
    /// an `.await`.
    ...
    /// Path to (or bare name of) the `holo` CLI binary, for `holo stop` (see `stop.rs`) and
    /// for `HoloBridge::restart_process` to respawn with the same binary reference.
    holo_bin: String,
}
...
impl HoloControlBridge {
    ...
    /// Swap in a freshly-constructed `A2aClient` (e.g. after `HoloBridge::restart_process`
    /// respawns `holo serve` with a new bearer token). In-flight turns started against the
    /// previous client keep running against it (each `handle_prompt` call clones the client
    /// once up front, see below) -- only turns started *after* the swap see the new one.
    pub fn replace_a2a_client(&self, client: A2aClient) {
        *self.client.write().expect("client lock poisoned") = client;
    }

    /// Clones the current `A2aClient` out of the lock. Called once per `handle_prompt`/
    /// `handle_stop` invocation rather than holding the lock for the request's duration --
    /// `A2aClient::clone()` is cheap (see its doc), and A2A calls are `.await`-heavy
    /// (streaming HTTP), so holding a lock across them would serialize concurrent turns for
    /// no benefit.
    fn client(&self) -> A2aClient {
    ...
    /// Path/bare-name of the `holo` CLI binary this bridge was built with -- exposed so the
    /// health-check supervisor (`crate::holo_bridge::health`) can respawn `holo serve` with
    /// the exact same binary reference after a crash, without needing its own separate copy
    /// of this config threaded through `main.rs`.
    pub fn holo_bin(&self) -> &str {
        &self.holo_bin
    }

    /// Publicly emit a [`ControlEvent::DaemonStatus`] -- the seam
    /// `crate::holo_bridge::health`'s crash-restart loop uses to report what happened, over
    /// the exact same `events_tx` sink (and therefore the exact same live control-channel
    /// connection, once one is mounted) as every A2A-derived event `emit` sends.
