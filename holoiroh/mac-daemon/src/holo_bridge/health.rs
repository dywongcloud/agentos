...
#[cfg(test)]
mod tests {
    use crate::holo_bridge::ControlEvent;

    /// `check_and_restart_if_needed` itself needs a live `HoloBridge` (which needs a real
    /// `holo serve` process to spawn against) to exercise end-to-end, so this test instead
    /// verifies the piece that's safe to check in isolation: that a `DaemonStatus` event
    /// constructed the way this module constructs it carries the text through unchanged, and
    /// that the `emit_daemon_status` seam it depends on works standalone (already covered by
    /// `control.rs`'s own tests) -- see `verify` for the live end-to-end run against a real
    /// `holo`-shaped subprocess.
    #[test]
    fn daemon_status_event_shape() {
        let event = ControlEvent::DaemonStatus {
            text: "Holo bridge (holo serve) restarted successfully.".to_string(),
        };
        match event {
            ControlEvent::DaemonStatus { text } => {
                assert!(text.contains("restarted successfully"));
            }
