...
///
/// `main.rs` holds this behind `Arc<HoloBridge>` (shared with the control channel and, now,
/// the health-check loop), so `process` is wrapped in a `tokio::sync::Mutex` -- the only field
/// that ever needs mutation after construction (a crash-restart replaces the running child).
/// `control`'s own interior mutability (`HoloControlBridge`'s `RwLock<mpsc::UnboundedSender<_>>`)
/// already follows this same "shared via Arc, mutated via an interior lock" shape.
...
impl HoloBridge {
    ...
    /// Non-blocking poll of whether the supervised `holo serve` child has exited. `Ok(None)` =
    /// still running; `Ok(Some(status))` = exited (the crash signal
    /// [`health::run_health_check_loop`] watches for). Briefly locks `process` (a `try_lock`
    /// would be sufficient in principle, but a real -- non-try -- lock is used since the only
    /// other holder, [`HoloBridge::restart_process`], releases quickly and running them
    /// strictly sequentially is exactly the behavior we want, not a reason to fail the poll).
    pub async fn try_wait_process(&self) -> std::io::Result<Option<std::process::ExitStatus>> {
        self.process.lock().await.try_wait()
    }

    /// Respawn `holo serve` after a detected crash, on the same port, using the same `holo`
    /// binary the bridge was originally started with. Rebuilds `control`'s `A2aClient` too
    /// (`HoloControlBridge::replace_event_sink` only swaps the outgoing-event sink; the A2A
    /// HTTP client itself is bound to the crashed process's bearer token, which
    /// `process::HoloServeProcess::spawn` always regenerates fresh on every spawn, so the old
    /// client would authenticate against a token that no longer exists).
    ///
    /// Deliberately touches only `self.process` -- there is no field on `HoloBridge`
    /// referencing the `iroh_live::Live` session or `crate::control_channel::ControlChannel`
    /// (both owned separately in `main.rs`), so this method structurally cannot reach, let
    /// alone tear down, the active P2P session.
    pub async fn restart_process(&self) -> Result<()> {
        let mut guard = self.process.lock().await;
        let port = guard.port;
        let holo_bin = self.control.holo_bin().to_string();

        tracing::info!(port, "restarting holo serve after detected crash");
        let new_process = HoloServeProcess::spawn(&holo_bin, port)
            .await
            .context("failed to respawn holo serve after crash")?;
        ...
        tracing::info!(
            streaming = card.streaming,
            protocol_version = ?card.protocol_version,
            "respawned holo serve agent card verified"
        );

        self.control.replace_a2a_client(client);
        *guard = new_process;
        Ok(())
    }
