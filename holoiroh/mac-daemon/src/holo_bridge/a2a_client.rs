...
// `Clone` is cheap: `reqwest::Client` is itself an `Arc`-backed handle internally (per its own
// docs: "you do not have to wrap it in an Rc or Arc to reuse it"), so cloning this struct is
// just cloning that handle plus two small `String`s. Added so `HoloControlBridge` can swap in
// a freshly-constructed client after a `holo serve` crash-restart (see
// `crate::holo_bridge::HoloBridge::restart_process`) without needing an `Option`/lock
// indirection around the client field itself.
#[derive(Clone)]
