...
impl ServerMessage {
    ...
    pub fn from_control_event(event: ControlEvent) -> Self {
        match event {
            ...
            // Daemon-wide status (e.g. `holo serve` crash-restart from
            // `holo_bridge::health`), not scoped to any request_id -- maps
            // directly onto the wire's `status` message, the same variant
            // used for e.g. the initial "control channel ready" greeting.
