...
mod tests {
    ...
    #[test]
    fn instruction_text_names_exact_settings_path() {
        assert!(MissingPermission::ScreenRecording
            .instruction()
            .contains("Screen Recording"));
        assert!(MissingPermission::Accessibility
            .instruction()
            .contains("Accessibility"));
    }

    #[test]
    fn preflight_result_ok_when_empty() {
        let result = PreflightResult { missing: vec![] };
        assert!(result.is_ok());
    }

    #[test]
    fn preflight_result_not_ok_when_populated() {
    ...
    #[test]
    fn preflight_result_both_missing_reports_both() {
        let result = PreflightResult {
            missing: vec![
                MissingPermission::ScreenRecording,
                MissingPermission::Accessibility,
        ...
        // report() writes to stderr; we only assert the data shape here
        // (that both entries survive construction, none short-circuited
        // away) -- the actual stderr output is exercised by running the
        // binary end-to-end.
    }

    // screen_recording_granted() / accessibility_granted() themselves are
    // not unit-tested here: they call into real macOS TCC state, which is
    // environment-dependent (CI/sandboxed runs will always read `false`).
    // They're exercised live via `cargo run` in the actual macOS
    // environment as part of this task's build/behavior witness.
}
