...
export async function agentTurn(args: {
  ...
  const system = [
    ...
    "- ZOHO CLIQ IS FIRST-PARTY: Zoho Cliq is NOT available via COMPOSIO_EXECUTE_TOOL / COMPOSIO_SEARCH_TOOLS, and it is NOT a Composio-registered app at all — it's a self-registered Zoho API Console client the operator set up directly (never say Composio owns or manages this integration; that is false). For anything Cliq (chats, messages, threads, scheduled messages, pins): zoho_cliq_status → (if not connected) zoho_cliq_connect (it sends the auth link itself — never write a Zoho URL) → zoho_cliq_action with a ZOHOCLIQ_* slug. On a zoho_cliq_action failure, read BOTH status and error before deciding: status===401 with error mentioning 'scope' means the Zoho API Console app registration itself isn't configured to allow that scope — check zoho_cliq_status's granted_scope field and tell the user PLAINLY which scope is missing so they (or whoever manages api-console.zoho.com for this app) can add it there; reconnecting will NOT fix this, do not loop zoho_cliq_connect for it. status===401 WITHOUT a scope-shaped error means a genuinely dead/revoked connection — use zoho_cliq_connect for that case only. ANY other status (400/403/404/etc) is a real action problem (bad chat_id, missing permission on that chat) — never reconnect for those either, surface the actual error instead. To message someone with no existing chat_id: there is no name/email lookup, try ZOHOCLIQ_LIST_CHATS first, else ask the user for that person's numeric Zoho Cliq user_id and use ZOHOCLIQ_CREATE_CHAT — never guess a user_id. Cliq event triggers (new message/chat/pin) exist as custom polling triggers — discover_triggers with toolkit 'zohocliq'.",
