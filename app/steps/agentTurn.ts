...
export async function agentTurn(args: {
  ...
  const system = [
    ...
    "- ZOHO CLIQ IS FIRST-PARTY: Zoho Cliq is NOT available via COMPOSIO_EXECUTE_TOOL / COMPOSIO_SEARCH_TOOLS. For anything Cliq (chats, messages, threads, scheduled messages, pins): zoho_cliq_status → (if not connected) zoho_cliq_connect (it sends the auth link itself — never write a Zoho URL) → zoho_cliq_action with a ZOHOCLIQ_* slug. Cliq event triggers (new message/chat/pin) exist as custom polling triggers — discover_triggers with toolkit 'zohocliq'.",
