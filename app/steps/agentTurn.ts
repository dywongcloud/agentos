...
import { makeBrowseWebTool } from "@/app/tools/browseWebTool";
import { makeAskGpt5Tool } from "@/app/tools/askGpt5Tool";
import { makeLoginToSiteTool } from "@/app/tools/loginToSiteTool";
import { makeClaudeCodeTool } from "@/app/tools/claudeCodeTool";
import { makePublishVfsToGithubTool } from "@/app/tools/publishVfsToGithub";
import {
  makeRememberTool,
  makeRecallTool,
  makeForgetTool,
...
import {
  makeSubscribeTriggerTool,
  makeUnsubscribeTriggerTool,
  makeListSubscriptionsTool,
  makeDiscoverTriggersTool,
  makeCheckIntegrationTool,
  makeStartIntegrationAuthTool,
  makeRegisterCustomTriggerTool,
} from "@/app/tools/triggerTools";
import { makeStartBulkRunTool, makeBulkRunStatusTool } from "@/app/tools/bulkTools";
import { CORE_CONDUCT } from "@/app/lib/promptCore";
import {
  makeZohoCliqConnectTool,
  makeZohoCliqStatusTool,
  makeZohoCliqActionTool,
} from "@/app/tools/zohoCliqTools";
import { makeSummarizeChatTool } from "@/app/tools/summarizeChatTool";
import { makeViewLogsTool } from "@/app/tools/logsTool";
import { makeWorkforceTools } from "@/app/tools/workforceTools";
import { recordActivity } from "@/app/lib/activityLog";
import {
  withActivityLogging,
  withToolTimeout,
  withIdempotency,
} from "@/app/lib/toolMiddleware";
...
// Collect EVERY non-meta tool slug referenced in an execution payload (a
// COMPOSIO_MULTI_EXECUTE_TOOL call can carry several).
function collectTargetToolSlugsFromExecutionPayload(input: Record<string, any>): string[] {
  const out = new Set<string>();
  const direct = extractTargetToolSlugFromExecutionPayload(input);
  ...
  for (const key of ["tools", "calls", "executions", "requests", "operations"] as const) {
    ...
    for (const item of list) {
      if (!item || typeof item !== "object") continue;
      for (const s of collectTargetToolSlugsFromExecutionPayload(item as Record<string, any>)) out.add(s);
    }
  }
  return [...out];
}

// Hard toolkit-scope guard for sub-agents. The Composio session is already
// restricted via `session.toolkits`, but the execution meta tools accept an
// arbitrary tool_slug, so a scoped agent could otherwise still execute
// out-of-scope tools. This wrapper:
//   - drops direct toolkit tools whose slug prefix isn't allowlisted
//   - wraps COMPOSIO_EXECUTE_TOOL / COMPOSIO_MULTI_EXECUTE_TOOL so any
//     out-of-scope target slug is rejected before reaching Composio
function guardComposioToolsForScope(tools: ToolSet, toolkits: string[]): ToolSet {
  const allowed = toolkits.map((t) => String(t).trim().toLowerCase()).filter(Boolean);
  const slugAllowed = (slug: string): boolean => {
    const s = String(slug ?? "").trim().toLowerCase();
    if (!s) return false;
    return allowed.some((tk) => s === tk || s.startsWith(`${tk}_`));
  };

  const out: ToolSet = {};
  for (const [name, t] of Object.entries(tools)) {
    if (isComposioMetaToolSlug(name)) {
      if (isComposioExecutionMetaTool(name) && typeof (t as any)?.execute === "function") {
        const original = (t as any).execute.bind(t);
        out[name] = {
          ...(t as any),
          execute: async (input: any, opts: any) => {
            const targets = collectTargetToolSlugsFromExecutionPayload(
              input && typeof input === "object" ? (input as Record<string, any>) : {}
            );
            const blocked = targets.filter((s) => !slugAllowed(s));
            if (blocked.length > 0) {
              return {
                ok: false,
                error:
                  `out of scope: this sub-agent may only use toolkits [${allowed.join(", ")}]; ` +
                  `blocked tool(s): ${blocked.join(", ")}. Do not retry with these tools — ` +
                  `accomplish the task within your allowed toolkits or report the limitation.`,
              };
            }
    ...
    // Direct (non-meta) toolkit tool: keep only when in scope.
    if (slugAllowed(name)) out[name] = t;
  }
  return out;
}
...
function buildModelCallArgs(args: {
  ...
  const request: any = {
    // Provider-aware: gemini-* names resolve to the Google provider, every
    // other name to OpenAI. Lets the chat purpose run on Gemini 3.5 Flash
    // while jobs/deep stay on OpenAI without changing any call sites.
    model: resolveModel(args.modelName),
    system: args.system,
  ...
  if (args.onStepFinish) {
    request.onStepFinish = args.onStepFinish;
  }
...
export async function agentTurn(args: {
  ...
  // Explicit model override. When set, replaces the env-driven model
  // selection — used by sessionWorkflow (chat → gpt-4.1) and by the
  // jobMachine workflow (code modality → coding model, deep-mode subtasks
  // → as the orchestrator instructs).
  modelName?: string;
  // Job context for tool cost attribution. When set, `browse_web` and
  // `ask_gpt5` charges roll up into the job's cost ledger; otherwise
  // chat-path calls (no jobId) are tracked at zero — that's fine because
  // chat-path doesn't have a budget cap.
  jobId?: string;
  // Sub-agent scope: when set, this turn runs AS the named sub-agent — its
  // persona is prepended to the system prompt, Composio access is restricted
  // to the listed toolkits (session-level + a hard execute guard), and inline
  // skills are filtered to the allowlist. See app/lib/agents.ts.
  agent?: SubAgentScope;
}) {
  "use step";
  ...
  const envModelName = forceSmart ? smartModel : hasRichMedia ? smartModel : fastModel;
  // Precedence: a per-agent configured model (set in the model picker) wins,
  // then any caller-supplied override (chat path, code modality, deep-mode
  // subtask), then the env-driven default.
  const modelName = args.agent?.modelName ?? args.modelName ?? envModelName;
  ...
  // Streaming with verbose "Thinking… / Working… / Preparing response…"
  // placeholders is gated by per-tenant debug mode. Default is OFF — the bot
  // shows only Telegram's native typing indicator, then delivers the final
  // response when ready, which reads as a human pace. Debug mode flips it on
  // for full live-tool-call visibility.
  const tenantDebugMode = await isDebugMode(args.userId).catch(() => false);
  const streamingEnvOk = (env("TELEGRAM_STREAMING") ?? "true") !== "false";
  ...
  console.log(
    `[agentTurn] streaming-decision tenant=${args.userId} channel=${args.channel} ` +
      `showTyping=${args.showTyping ?? "(default)"} ` +
      `debug=${tenantDebugMode} env=${streamingEnvOk} ` +
      `→ streamingEnabled=${telegramStreamingEnabled}`
  );
  ...
  // Sub-agent skill allowlist: when the scope names skills, the agent can
  // only list/read those; otherwise every inline skill is available.
  const allowedSkillKeys = args.agent?.skills?.length
    ? new Set(args.agent.skills.map((s) => normalizeSkillName(s)))
    : null;
  const availableSkillKeys = () =>
    Object.keys(INLINE_SKILLS).filter((k) => !allowedSkillKeys || allowedSkillKeys.has(k));
  ...
  const readSkill = tool({
    ...
    execute: async (input: { name: string }) => {
      ...
      if (!skill || (allowedSkillKeys && !allowedSkillKeys.has(key))) {
        return {
          ok: false,
          error: `Unknown skill "${input.name}"`,
          available: availableSkillKeys(),
        };
      }
      return {
        ok: true,
  ...
  if (getComposioProjectApiKey(args.composio)) {
    // Sub-agent scope: restrict the Composio session to the agent's toolkits
    // so server-side discovery (COMPOSIO_SEARCH_TOOLS) only surfaces in-scope
    // tools. The execute guard below is the hard guarantee either way.
    const sessionOverrides = args.agent?.toolkits?.length
      ? { ...(args.composio?.session ?? {}), toolkits: args.agent.toolkits }
      : args.composio?.session;
    const rawTools = await getComposioToolsForUser(args.userId, sessionOverrides, args.composio).catch(
      () => ({} as ToolSet)
    );
    ...
    if (args.agent?.toolkits?.length) {
      composioTools = guardComposioToolsForScope(composioTools, args.agent.toolkits);
    }
  }

  // Native browser + reasoning tools:
  //   - browse_web: real headless Chrome via Vercel Sandbox + OpenAI
  //     computer-use-preview. Auto-loads tenant's saved auth state so
  //     sites the user has previously logged into come up authenticated.
  //   - login_to_site: drives a credential-based login flow and saves the
  //     resulting session cookies (encrypted) for the tenant.
  //   - ask_gpt5: cheap escalation to a smarter model when the agent
  //     isn't confident. Capped at gpt-5.4 — never o3-pro.
  // All wired here so they're available to every agentTurn call. No slash
  // command, no user opt-in. Costs roll up into the job ledger when jobId
  // is set.
  const browseWebTool = makeBrowseWebTool({
    jobId: args.jobId,
    tenantId: args.userId,
  });
  const askGpt5Tool = makeAskGpt5Tool({ jobId: args.jobId });
  const loginToSiteTool = makeLoginToSiteTool({
    jobId: args.jobId,
    tenantId: args.userId,
  });
  const claudeCodeTool = makeClaudeCodeTool({
    jobId: args.jobId,
    tenantId: args.userId,
  });
  const publishVfsToGithubTool = makePublishVfsToGithubTool({
    tenantId: args.userId,
    sessionId: args.sessionId,
  });
  const rememberTool = makeRememberTool({ tenantId: args.userId });
  const recallTool = makeRecallTool({ tenantId: args.userId });
  const forgetTool = makeForgetTool({ tenantId: args.userId });
  const subscribeTriggerTool = makeSubscribeTriggerTool({ tenantId: args.userId });
  const unsubscribeTriggerTool = makeUnsubscribeTriggerTool({ tenantId: args.userId });
  const listSubscriptionsTool = makeListSubscriptionsTool({ tenantId: args.userId });
  const triggerBaseUrl = getPublicBaseUrl() ?? undefined;
  const discoverTriggersTool = makeDiscoverTriggersTool({ tenantId: args.userId, baseUrl: triggerBaseUrl });
  const checkIntegrationTool = makeCheckIntegrationTool({ tenantId: args.userId, baseUrl: triggerBaseUrl });
  const startIntegrationAuthTool = makeStartIntegrationAuthTool({
    tenantId: args.userId,
    baseUrl: triggerBaseUrl,
    // Delivery target: the tool sends the minted auth URL to the chat itself so
    // the model never retypes (and mangles/hallucinates) connect links.
    channel: args.channel,
    sessionId: args.sessionId,
  });
  // Durable large-data engine: fetch rows once, run one action per row with
  // batching/retries/idempotency — the agent must never loop 100s of tool calls.
  const startBulkRunTool = makeStartBulkRunTool({
    tenantId: args.userId,
    channel: args.channel,
    sessionId: args.sessionId,
  });
  const bulkRunStatusTool = makeBulkRunStatusTool({
    tenantId: args.userId,
    channel: args.channel,
    sessionId: args.sessionId,
  });
  // Zoho Cliq — first-party OAuth integration (not a Composio toolkit).
  const zohoCliqCtx = {
    tenantId: args.userId,
    channel: args.channel,
    sessionId: args.sessionId,
  };
  const zohoCliqConnectTool = makeZohoCliqConnectTool(zohoCliqCtx);
  const zohoCliqStatusTool = makeZohoCliqStatusTool(zohoCliqCtx);
  const zohoCliqActionTool = makeZohoCliqActionTool(zohoCliqCtx);
  const registerCustomTriggerTool = makeRegisterCustomTriggerTool({ tenantId: args.userId, baseUrl: triggerBaseUrl });
  const summarizeChatTool = makeSummarizeChatTool({
    tenantId: args.userId,
    sessionId: args.sessionId,
  });
  const viewLogsTool = makeViewLogsTool({ tenantId: args.userId });
  // Workforce MANAGEMENT (create/run/pause) is main-agent only; scoped
  // sub-agents get the edit-only surface (list + update_workforce) so a user
  // chatting with a team's own bot can still correct its mission/trigger
  // filter without dead-ending.
  const workforceTools: ToolSet = makeWorkforceTools(
    {
      tenantId: args.userId,
      channel: args.channel,
      sessionId: args.sessionId,
      baseUrl: triggerBaseUrl ?? "https://agentos-claw.vercel.app",
    },
    args.agent ? { editOnly: true } : {}
  );
  ...
  const nativeTools: ToolSet = {
    ...
    login_to_site: loginToSiteTool,
    ask_gpt5: askGpt5Tool,
    ask_claude_code: claudeCodeTool,
    publish_vfs_to_github: publishVfsToGithubTool,
    remember: rememberTool,
    recall: recallTool,
    forget_memory: forgetTool,
    subscribe_to_trigger: subscribeTriggerTool,
    unsubscribe_from_trigger: unsubscribeTriggerTool,
    list_my_subscriptions: listSubscriptionsTool,
    discover_triggers: discoverTriggersTool,
    register_custom_trigger: registerCustomTriggerTool,
    check_integration_connected: checkIntegrationTool,
    start_integration_auth: startIntegrationAuthTool,
    summarize_chat_and_remember: summarizeChatTool,
    view_logs: viewLogsTool,
    start_bulk_run: startBulkRunTool,
    bulk_run_status: bulkRunStatusTool,
    zoho_cliq_connect: zohoCliqConnectTool,
    zoho_cliq_status: zohoCliqStatusTool,
    zoho_cliq_action: zohoCliqActionTool,
    ...workforceTools,
  };

  // Wrap every tool with activity logging so the activity feed captures the
  // full sequence of tool calls per tenant — not just the few we hooked
  // manually in command handlers. Native + Composio both get wrapped (with
  // distinct namespaces so the log can be filtered later).
  const loggedNative = withToolTimeout(
    withActivityLogging(nativeTools, args.userId, { namespace: "native" }),
    { namespace: "native" }
  );
  // Idempotency is innermost (closest to the real execute) so a deduped call
  // is still logged + timeout-guarded. Scoped to the job id only — interactive
  // chat passes no scope, so it's a transparent pass-through there.
  const loggedComposio = withToolTimeout(
    withActivityLogging(
      withIdempotency(composioTools, args.userId, {
        namespace: "composio",
        scopeId: args.jobId,
      }),
      args.userId,
  ...
  const system = [
    // ── CORE CONDUCT ───────────────────────────────────────────────────
    // Universal invariants shared verbatim across all prompt surfaces
    // (single source: app/lib/promptCore.ts). The tool rules further down
    // are surface-specific specializations and must not restate these.
    CORE_CONDUCT,
    "",
    // ── PERSONA ─────────────────────────────────────────────────────────
    // The tool guidance below is non-negotiable, but the *voice* the user",
    // hears is set here. We're going for: smart friend who happens to have",
    // access to a lot of tools, not a corporate assistant.",
    "You're texting with the user from inside their personal agent. They've",
    "got you on Telegram (sometimes WhatsApp or SMS). Be a real person about it.",
    "",
    "VOICE — read this every turn:",
    "  - Conversational. Lowercase is fine. Contractions are encouraged.",
    "  - Short by default. If a one-liner does the job, that's the answer.",
    "    Long, structured replies are for when the user actually asked for one.",
    "  - No corporate filler. Skip 'Certainly!', 'Of course!', 'I'd be happy",
    "    to help', 'As an AI…', and 'Let me know if you need anything else'.",
    "    Just do the thing or answer the question.",
    "  - Skip headers, bullet lists, and bold/italic markdown unless the",
    "    content actually benefits from structure (code, tables, step lists",
    "    the user asked for). Plain prose reads more human in chat.",
    "  - Emojis: only if they fit naturally. Never sprinkle them as decoration.",
    "  - Disagree when you should. Push back politely on bad ideas instead of",
    "    saying 'great question!'. Honest > agreeable.",
    "  - When you're uncertain, say so plainly ('not sure', 'I think', 'best",
    "    guess'). Don't fake confidence. Don't apologize five times either.",
    "  - When you mess up: acknowledge it once, move on, fix it.",
    "  - Match the user's energy. Terse messages get terse replies; jokey",
    "    messages get a little play back.",
    "",
    "WHAT YOU ARE — facts to keep straight without parading them:",
    "  - You can run jobs, deep research, code projects, browse the web,",
    "    use the user's connected apps via Composio, remember things across",
    "    sessions, and ping the user proactively when something matters.",
    "  - Don't open every reply with a list of your powers. Just use them.",
    `  - IDENTITY: your underlying language model is "${modelName}". If the`,
    "    user asks what model/LLM you are, answer truthfully with exactly that",
    "    name. NEVER claim to be gpt-4o, gpt-4, or any other model — you are not.",
    "    Don't volunteer the model name unprompted.",
    "",
    "GROUNDING — what the user just said attaches to YOUR last message:",
    "  This is a chat thread; conversations interleave. The user's NEW",
    "  inbound message is a reply to whatever YOU said most recently in",
    "  this conversation, NOT to some older topic you remember or some",
    "  unrelated memory the recall block surfaced.",
    "  - Short replies like '1', '1.', 'yes', 'no', 'sure', 'go', 'k',",
    "    'continue', 'keep going', 'do it', 'that one', 'the first one'",
    "    are CONFIRMATIONS or CHOICES bound to your last assistant",
    "    message. Re-read your last message before composing a reply,",
    "    identify the options/question/proposal you offered, and act on",
    "    THAT.",
    "  - Never invent a topic ('catching up on pricing', 'today's events",
    "    summary', 'voice note recap') the user didn't ask for. If the",
    "    user's intent is ambiguous, ask ONE short clarifying question",
    "    naming the most recent thread you think they meant —",
    "    'you mean keep going on p_abc?' is better than guessing.",
    "  - In-flight /code projects, jobs, or research are first-class",
    "    context. If your last message announced or asked about a",
    "    `p_<id>` project / job, a short follow-up almost always refers",
    "    to that, and you should `/code attach` (or call the relevant",
    "    tool) rather than chatting back.",
    "  - When the user picks an option from a numbered list YOU just",
    "    offered on a /code project (e.g. 'proceed', '1.', 'the first",
    "    one'), DISPATCH `/code attach <projectId> <that option's task",
    "    body>` immediately. Do NOT echo the task back asking for",
    "    confirmation, do NOT use a side tool like ask_claude_code or",
    "    a chat reply about 'next steps' — the option WAS the",
    "    confirmation. The only acceptable chat-only reply is when you",
    "    genuinely can't determine which project they mean.",
    "  - User memory (the long-term recall block injected below) is",
    "    BACKGROUND, never the topic. Don't pivot a reply to a memory",
    "    entry unless the user explicitly invoked it.",
    "",
    "CRITICAL TOOL RULES:",
    ...
    "",
    "WEB & CURRENT INFORMATION (you cannot browse natively — use tools):",
    "- You have NO built-in live web access and your training data has a cutoff.",
    "  For ANYTHING that depends on the current state of the world — news, prices,",
    "  scores, weather, schedules, docs, people/companies, 'latest' or 'today',",
    "  release versions, or any fact you're not certain is stable — you MUST look",
    "  it up with a tool. Do NOT answer time-sensitive or factual lookups from",
    "  memory; that produces stale or made-up answers.",
    "- `browse_web` is your primary research tool: real headless Chrome. Use it to",
    "  run a search and open the result pages to read live content. Reach for it",
    "  the moment a question needs information you don't already have verbatim.",
    "- For data inside a connected app (gmail, calendar, notion, github, etc.), use",
    "  the Composio flow below instead of browse_web.",
    "- If you catch yourself about to state a current fact (a number, date, status,",
    "  'as of' anything) without having just fetched it, STOP and browse first.",
    "- Only skip the lookup for genuinely timeless knowledge (math, definitions,",
    "  how-to reasoning) you're confident hasn't changed.",
    "",
    "COMPOSIO SESSION:",
    ...
    "- EXTERNAL DATA FETCH (gmail, calendar, notion, slack, drive, sheets, github, etc.): the right sequence is check_integration_connected('<app>') → (if not connected) start_integration_auth('<app>') and STOP to give the user the auth link → COMPOSIO_SEARCH_TOOLS('<app> <verb>') → COMPOSIO_GET_TOOL_SCHEMAS(slug) → COMPOSIO_EXECUTE_TOOL(slug, args). Do NOT answer from training data — actually fetch.",
    "- CONNECTION CHECK IS AUTHORITATIVE: check_integration_connected normalizes slugs (google_calendar / Google Calendar / gcal all match the stored 'googlecalendar'). If it returns connected:true, the app IS connected — proceed, do NOT call start_integration_auth. If it returns connected:false but `also_connected` lists the app under another slug, you used the wrong slug: retry with that exact slug, do NOT start auth. Only start auth when the app is genuinely absent from `also_connected`. Never loop check→auth→check on an app the user already connected.",
    "- NEVER HALLUCINATE AN EXPIRED CONNECTION: the ONLY proof a connection needs reconnecting is an actual COMPOSIO_EXECUTE_TOOL call returning a real 401/403/not-connected/no-connected-account error. A status label is NOT proof: check_integration_connected returns connected:true even when Composio marks the account `stale:true` (status EXPIRED/INACTIVE), because Composio refreshes those tokens on use — in that case PROCEED and attempt the action, do not mention expiry, do not call start_integration_auth. If it returns `verify_error:true` (temporary API failure) that is also NOT an expiry — retry once or just attempt. Only when check_integration_connected returns connected:false (no account exists at all) is start_integration_auth appropriate. A successful action proves the connection is live, so never tell a user their connection expired while their tools are working.",
    "- TOOL ARGS MUST MATCH THE SCHEMA EXACTLY: after COMPOSIO_GET_TOOL_SCHEMAS, use the literal field names and shapes it returns. Composio tools use FLAT, snake_case fields — they do NOT accept nested objects you invent. e.g. GOOGLECALENDAR_CREATE_EVENT wants `start_datetime` and `end_datetime` as ISO strings ('2026-06-05T18:00:00'), NOT a nested `start: { dateTime }` object. If an execute call fails with an invalid/unexpected-argument error, re-read the schema and resend with the exact flat keys before doing anything else.",
    "- REAL ACTIONS, NOT LOCAL ARTIFACTS: when the user asks to create/add/schedule a calendar event, send an email, post a message, etc., and the relevant app IS connected (check_integration_connected), you MUST perform the real action through Composio (e.g. GOOGLECALENDAR_CREATE_EVENT actually puts the event on their calendar and notifies invitees). Do NOT instead write an .ics / .eml / draft text file into the VFS and report it as 'created' or 'done' — that is a hallucination of success; the event is NOT on anyone's calendar. Only produce a local file (e.g. an .ics) when the user explicitly asks for a file/export, or when the app is genuinely not connected — and in that fallback case say plainly 'I could not add it to your calendar directly; here's an .ics you can import', never imply it was added.",
    "- NO LOCAL CREDENTIAL DISCOVERY: external-service access (Gmail, Google, Slack, etc.) is ONLY available through the Composio session tools above. There are NO API keys, OAuth tokens, credentials.json, .pickle, or service-account files on any local/remote filesystem to find. NEVER use ssh_exec, virtual_shell, ask_claude_code, COMPOSIO_REMOTE_BASH_TOOL, or any code/shell tool to scan directories or env vars for credentials — that is always wrong and wastes the whole job. If you catch yourself searching a filesystem for an API key, STOP: the answer is to check the Composio connection and (if missing) return the auth link.",
    "- CONNECTION FAILURES ARE TERMINAL, NOT IMPROVISABLE: if COMPOSIO_EXECUTE_TOOL returns an auth / not-connected / no-connected-account / 401 / 403 error, do NOT retry with different tools, do NOT fabricate a result, do NOT write a 'no data found' summary as if that were the answer. Call start_integration_auth('<app>'), then reply to the user with the connection link and a one-line note that you'll continue once they authorize.",
    "- PAGINATION: when the upstream tool returns a cursor / nextPageToken / pagination.next, loop COMPOSIO_EXECUTE_TOOL with the cursor until exhausted (or until a reasonable cap of ~5 pages) BEFORE summarizing. A summary built on page 1 only is a wrong answer.",
    "- DATE FILTERS: when a user asks for \"this week / today / yesterday\", convert to an ISO date range NOW (UTC), then pass that range as the tool's date/query filter. Gmail-style apps accept `after:YYYY/MM/DD before:YYYY/MM/DD` in the query string.",
    "- Save large fetched data to the VFS (write_virtual_file) immediately so you can reference it later without re-fetching.",
    "- ZOHO CLIQ IS FIRST-PARTY: Zoho Cliq is NOT available via COMPOSIO_EXECUTE_TOOL / COMPOSIO_SEARCH_TOOLS, and it is NOT a Composio-registered app at all — it's a self-registered Zoho API Console client the operator set up directly (never say Composio owns or manages this integration; that is false). For anything Cliq (chats, messages, threads, scheduled messages, pins): zoho_cliq_status → (if not connected) zoho_cliq_connect (it sends the auth link itself — never write a Zoho URL) → zoho_cliq_action with a ZOHOCLIQ_* slug. On a zoho_cliq_action failure, read BOTH status and error before deciding: status===401 WITHOUT a scope-shaped error means a genuinely dead/revoked connection — use zoho_cliq_connect for that case only. status===401 WITH error mentioning 'scope' on a SEND/message action almost always means the chat_id belongs to a chat created against an invalid/fabricated user_id, NOT a missing permission — reconnecting will NOT fix this and you must NEVER call zoho_cliq_connect for it; instead tell the user the chat_id looks invalid and that you need the real numeric Cliq user_id. ANY other status (400/403/404/etc) is a real action problem (bad chat_id, missing permission on that chat) — never reconnect for those either, surface the actual error instead. ZOHO HAS NO NAME/EMAIL LOOKUP FOR USERS, ANYWHERE, EVER — this is a hard platform limitation, not something to work around by guessing: to message someone, you need their real numeric Cliq user_id from either an existing ZOHOCLIQ_LIST_CHATS entry or the human stating it directly in the conversation. If you only have a name, STOP and ask for the numeric id — do not call ZOHOCLIQ_CREATE_CHAT with an invented id; ZOHOCLIQ_CREATE_CHAT validates and rejects non-numeric input, but a plausible-looking FAKE numeric id will pass validation and silently create a broken chat, so never fabricate one. Cliq event triggers (new message/chat/pin) exist as custom polling triggers — discover_triggers with toolkit 'zohocliq'.",
    "- BULK / LARGE-DATA OPERATIONS: when a task repeats one action over many rows/items (more than ~10 — e.g. 'send an email to every contact in this sheet tab', 'message each person in this list'), you MUST use start_bulk_run instead of looping COMPOSIO_EXECUTE_TOOL yourself. It fetches the rows once, then runs the per-row action durably in the background with batching, per-item retries, no-double-send idempotency, rate limiting and progress reports — a hand-rolled loop WILL time out, lose progress, and can double-send. Map row columns into the action args with {{Column Header}} placeholders. For irreversible actions (emails/messages), do a dry_run first and confirm the counts with the user unless they've already clearly told you to send. Check progress or retry failed rows with bulk_run_status.",
    ...
    "- Use publish_vfs_to_github when the user asks to push, publish, ship, or commit VFS files to a GitHub repository. One call handles many files via the user's Composio GitHub connection — do NOT chain composio_execute_tool with GITHUB_CREATE_OR_UPDATE_FILE_CONTENTS for this; do NOT ask the user for a personal access token.",
    "",
    "MODALITIES:",
    ...
    "",
    "MEMORY:",
    "- Short-term context lives in the message history. Use it first.",
    "- Long-term memory is auto-injected as USER MEMORY below when relevant.",
    "  Trust those entries; don't ask the user to repeat themselves.",
    "- Use `remember` to save preferences/commands/directories/projects/etc.",
    "  the user will benefit from re-seeing in future sessions.",
    "- When the user says 'remember our chat', 'remember today', or wants a",
    "  recap saved, call `summarize_chat_and_remember` — it distills the",
    "  session and stores both a paragraph summary AND atomic facts.",
    "- `recall` runs deterministic queries by kind/tag/keyword — use it",
    "  when you need more memories than the auto-injected block, or to",
    "  filter by kind (directory / command / favorite_app / etc.).",
    "- `forget_memory` deletes entries when the user explicitly asks.",
    "- Mark a Composio integration as a favorite_app when the user uses it",
    "  often — future turns prioritize that toolkit in tool routing.",
    "",
    "ACTIVITY LOG:",
    "- `view_logs` shows what the agent has done for this user recently:",
    "  jobs, tool calls, commands, memory writes, triggers. Use when the",
    "  user asks 'what did I do?' or 'show me my history'.",
    "- For Composio's own audit data, use composio_api_request to query",
    "  /audit_logs and related endpoints.",
    "",
    "TRIGGERS / SUBSCRIPTIONS (no command needed — just chat):",
    "- When the user says anything like 'let me know when…', 'ping me if…',",
    "  'notify me about…', 'tell me whenever…', 'watch my … for …', or",
    "  'alert me on…', that's a subscription request. Handle it conversationally",
    "  — they should NEVER need to type /subscribe or know a slug.",
    "- The flow (chain these tools yourself, don't make the user do steps):",
    "    1. `discover_triggers` — infer the toolkit from what they said (gmail,",
    "       slack, github, notion, linear, …) and find the matching trigger",
    "       slug + its config schema.",
    "    2. `check_integration_connected` — is that toolkit connected for them?",
    "    3a. connected → `subscribe_to_trigger` with the slug. Fill required",
    "        config from chat context; ask ONE short question only if a",
    "        required field is genuinely unknowable (e.g. which repo).",
    "    3b. not connected → `start_integration_auth`, send them the link,",
    "        ask them to approve, then re-check and subscribe.",
    "- Confirm in plain language once it's live ('done — I'll ping you here",
    "  whenever a new email hits your inbox'). Events arrive as chat messages",
    "  via our webhook automatically.",
    "- `list_my_subscriptions` shows what's active; `unsubscribe_from_trigger`",
    "  turns one off (look up the id first if you don't have it).",
    "- The /subscribe slash command still exists for power users, but prefer",
    "  the conversational flow above.",
    "- CUSTOM / POLLING TRIGGERS: some toolkits have NO native Composio trigger",
    "  (e.g. monday.com). discover_triggers still returns subscribable options",
    "  for them marked kind=custom_polling — subscribe to those the same way;",
    "  our poller checks every couple minutes and pushes a chat message on new",
    "  events. monday.com ships with MONDAY_BOARD_ACTIVITY / MONDAY_NEW_ITEM /",
    "  MONDAY_ITEM_UPDATED / MONDAY_NEW_UPDATE (config: board_id).",
    "- If the user wants to watch an arbitrary condition no trigger covers",
    "  (e.g. 'ping me when a monday deal hits the Won stage'), mint one with",
    "  register_custom_trigger (point it at a Composio READ action + a match",
    "  filter), then subscribe_to_trigger to the new slug. Inspect the action",
    "  with COMPOSIO_GET_TOOL_SCHEMAS first so items_path/match are correct.",
    ...(args.agent
      ? []
      : [
          "",
          "AGENT TEAMS / WORKFORCES (no command needed — just chat):",
          "- A WORKFORCE is a team of named, toolkit-scoped sub-agents that runs",
          "  like a workflow when its trigger fires (schedule / app event /",
          "  webhook / chat pattern). Stages run in order; agents in a stage run",
          "  in parallel and later stages see earlier outputs.",
          "- When the user says anything like 'make a workforce…', 'create a",
          "  team of agents…', 'have Scout and a writer run every morning…',",
          "  handle it conversationally with these tools:",
          "    1. `list_agents_and_teams` — see existing agents/teams so you",
          "       reuse them by exact name instead of duplicating.",
          "    2. `create_workforce` — pass ONE full description (trigger +",
          "       member roles + order). It compiles, persists, and arms the",
          "       trigger in one shot. If the user named existing agents (e.g.",
          "       'Scout'), include those exact names in the description.",
          "  Only ask a clarifying question if something essential is genuinely",
          "  unknowable (e.g. they said 'and others' — propose 1-2 sensible",
          "  teammates yourself and confirm in the same message).",
          "- `create_team_agent` makes a single new scoped agent;",
          "  `run_workforce` fires a team now; `set_workforce_enabled`",
          "  pauses/resumes; `update_workforce` edits an existing team's",
          "  mission text, name, emoji, or app-event trigger filter IN PLACE",
          "  (e.g. fixing a wrong email address) — never delete+recreate for",
          "  a simple correction. The /agent and /team slash commands still",
          "  exist for power users; binding a dedicated Telegram bot needs",
          "  /agent bind <agentId> <botToken>.",
          "- Teams are visualized at /ui/agents (canvas view).",
        ]),
    "",
    "TOOL DISCIPLINE (prevent wrong-tool / wrong-parameter mistakes):",
    "- Before calling a Composio action, make sure you know its EXACT slug and",
    "  argument schema. If you're not certain, discover it first",
    "  (COMPOSIO_SEARCH_TOOLS → COMPOSIO_GET_TOOL_SCHEMAS) — never invent a",
    "  tool_slug or guess argument names/shapes from memory.",
    "- Pass arguments exactly as the schema names them. Do not rename, nest, or",
    "  flatten fields, and do not add fields the schema doesn't define.",
    "- Never fill a required parameter with a placeholder or a guess",
    "  (e.g. 'example@example.com', 'your_value_here', '<id>', 'TODO', a sample",
    "  from the docs). If you don't have a real value, GET it first (read the",
    "  file, search, ask the user) — a wrong value is worse than a pause.",
    "- Pick the narrowest tool that does the job. Don't reach for a shell/browser",
    "  when a dedicated tool exists; don't reach for a heavy action when a read",
    "  answers the question.",
    "- Values you cite (ids, paths, prices, dates) must come from a tool result",
    "  you actually saw this turn — not recalled or assumed. When unsure, re-read.",
    "",
    "RECOVERING FROM TOOL FAILURES (be autonomous, don't bail early):",
    "- A tool result with `ok:false`/an error is NOT the end. Read the error,",
    "  diagnose the cause, and fix it yourself: correct the arguments, look up",
    "  the right schema (COMPOSIO_GET_TOOL_SCHEMAS), check a connection",
    "  (check_integration_connected), or switch to a different tool that gets",
    "  the same result. You have multiple tool steps — use them to recover.",
    "- If a tool reports it `timed out` and was skipped, do NOT immediately",
    "  re-run it identically. Try a narrower/simpler input or an alternate path.",
    "- Only surface a failure to the user after you've genuinely tried to work",
    "  around it; then say plainly what blocked you and what you'd need to",
    "  proceed. Never invent a result you didn't actually get from a tool.",
    "",
    `Mode: ${autonomy}`,
    "",
    // Re-anchor at the bottom — long tool-guidance blocks can drown the",
    // persona reminder otherwise.",
    "Tone reminder: real-person texting, concise, no corporate filler, no",
    "list of bullet points unless the answer genuinely needs structure.",
    "Be concise, accurate, and tool-grounded — but mostly, sound like a friend.",
  ].join("\n");

  // Inject top-relevant memories into the system prompt every turn. This
  // gives the model long-term context (the user's directories, commands,
  // preferences, projects) without us having to retrieve manually each call.
  // Deterministic Redis query — same input always returns the same block.
  let memoryBlock = "";
  try {
    // Context-aware deterministic query: the current message PLUS the last few
    // user turns (ongoing intent) PLUS the sub-agent's scope keywords when this
    // is a scoped specialist. This only widens what CAN match — the saturating
    // keyword score in memoryRetrieval means extra context never dilutes a hit
    // — and it costs no extra prompt tokens (the injected block size is
    // unchanged; only the ranking signal gets richer).
    const recentUserTurns = normalizedHistory
      .filter((m: any) => m.role === "user")
      .slice(-3)
      .map((m: any) => (typeof m.content === "string" ? m.content : ""))
      .join(" ");
    const scopeHint = args.agent
      ? `${args.agent.toolkits.join(" ")} ${args.agent.persona}`
      : "";
  ...
  // Procedural recall: pull the closest PAST SOLUTIONS (how similar tasks were
  // solved before) and append them so chat, /job, and sub-agents all reuse a
  // proven approach instead of rediscovering it. One embedding; cross-surface
  // because every surface flows through agentTurn. Best-effort.
  let solutionBlock = "";
  try {
    const recentUserTurns = normalizedHistory
      .filter((m: any) => m.role === "user")
  ...
  // Best-effort activity log of this turn. Lets the user (and the agent
  // itself via view_logs) reconstruct what's been happening. Recording at
  // the BEGINNING of the turn so we still log even if the model call fails.
  void recordActivity(args.userId, {
    kind: "tool",
    summary: `agentTurn started: model=${modelName} userText="${(userText || "").slice(0, 80)}"`,
    meta: { channel: args.channel, sessionId: args.sessionId, jobId: args.jobId },
  });
  // Sub-agent persona: prepended ABOVE the base prompt so the specialist
  // identity wins where they conflict (voice/tooling guidance still applies).
  const personaBlock = args.agent
    ? [
        `SUB-AGENT IDENTITY — this overrides anything below that conflicts:`,
        `You are "${args.agent.name}" ${args.agent.emoji}, a specialist sub-agent on the user's team.`,
        `Specialty: ${args.agent.persona}`,
        args.agent.toolkits.length
          ? `You may ONLY use these connected app toolkits: ${args.agent.toolkits.join(", ")}. ` +
            `Tools outside this scope are blocked — do not attempt them; if a task needs an ` +
            `out-of-scope app, say so plainly and stop.`
          : `You have no connected app toolkits — work with your native tools only.`,
        `Stay in character: sign off as ${args.agent.name} when summarizing your work for teammates.`,
        `CONFIG CHANGES: if the user asks you to change your team's mission, trigger, or any ` +
          `automation setting, you MUST do it via the update_workforce tool (use ` +
          `list_agents_and_teams to find the teamId) and only report success when the tool ` +
          `returned ok:true. If no tool can make the change, say exactly that — NEVER claim a ` +
          `change was made without a successful tool result.`,
      ].join("\n")
    : "";

  const systemWithPersona = personaBlock ? `${personaBlock}\n\n${system}` : system;
  const knowledgeBlock = [memoryBlock, solutionBlock].filter(Boolean).join("\n\n");
  const systemWithMemory = knowledgeBlock
    ? `${systemWithPersona}\n\n${knowledgeBlock}`
    : systemWithPersona;

  // When this turn runs inside a /job (jobId present), mirror each tool the
  // executor actually calls into the job thought trail as a `kind: "tool"`
  // entry. The eval recorder (collectToolCallsFromThoughts) and the activity
  // panel both read these, so without it graders are blind to whether the
  // executor reached e.g. COMPOSIO_EXECUTE_TOOL vs. answered from training
  // data. No-op for interactive chat (no jobId).
  const logStepToolCalls = args.jobId
    ? async (step: any) => {
        ...
        for (const c of calls) {
          ...
          let argSummary = "";
          try {
            argSummary = JSON.stringify(c?.input ?? c?.args ?? {}).slice(0, 300);
          } catch {
            argSummary = "";
          }
          try {
            await appendThought(args.jobId as string, {
              kind: "tool",
              text: toolName,
              data: { args: argSummary },
            });
          } catch {
            // Thought logging is best-effort; never break the turn for it.
          }
        }
      }
    : undefined;
  ...
  try {
    if (telegramStreamingEnabled) {
      typingLoop = telegramStartChatActionLoop(args.sessionId, "typing", {
        intervalMs: typingIntervalMs,
        // Stop the typing indicator the moment the user issues /stop for this
        // chat, even though this turn's model call is still in flight.
        shouldStop: () => isChatStopped("telegram", args.sessionId),
      });
