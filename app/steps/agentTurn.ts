...
export async function agentTurn(args: {
  ...
  const system = [
    ...
    "- ZOHO CLIQ IS FIRST-PARTY: Zoho Cliq is NOT available via COMPOSIO_EXECUTE_TOOL / COMPOSIO_SEARCH_TOOLS. For anything Cliq (chats, messages, threads, scheduled messages, pins): zoho_cliq_status → (if not connected) zoho_cliq_connect (it sends the auth link itself — never write a Zoho URL) → zoho_cliq_action with a ZOHOCLIQ_* slug. A zoho_cliq_action failure with status===401 means reconnect; ANY other status (400/403/404/etc) is a real action problem (bad chat_id, missing permission) — do NOT call zoho_cliq_connect for those, it will not fix them, surface the actual error instead. To message someone with no existing chat_id: there is no name/email lookup, try ZOHOCLIQ_LIST_CHATS first, else ask the user for that person's numeric Zoho Cliq user_id and use ZOHOCLIQ_CREATE_CHAT — never guess a user_id. Cliq event triggers (new message/chat/pin) exist as custom polling triggers — discover_triggers with toolkit 'zohocliq'.",
